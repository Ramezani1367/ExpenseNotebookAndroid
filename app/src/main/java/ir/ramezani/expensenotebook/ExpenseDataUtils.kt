...
object ExpenseDataUtils {
    ...
    fun updateAllWidgets(context: Context) {
        ...
        // همهٔ providerها باید هم‌زمان به‌روزرسانی شوند. ویجت قابل تغییر اندازه
        // از ExpenseWidgetProvider استفاده می‌کند، اما ویجت‌های ثابت هرکدام
        // receiver جداگانه دارند و قبلاً در اینجا جا می‌افتادند.
        ...
        providers.forEach { provider ->
            ...
            if (ids.isNotEmpty()) {
                when (provider) {
                    ExpenseWidgetProvider::class.java -> ExpenseWidgetProvider.updateWidgets(context, manager, ids)
                    else -> FixedWidgetProvider.updateWidgets(context, manager, ids, provider)
                }
            }
        }
    }
