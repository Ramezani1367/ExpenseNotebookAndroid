...
class QuickAddActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        titleInput = makeInput("عنوان هزینه")
        priceInput = makeInput("مبلغ").apply { inputType = android.text.InputType.TYPE_CLASS_NUMBER }
        ...
        val foot = TextView(this).apply {
            text = "بلافاصله ثبت می‌شه و ویجت همونجا آپدیت می‌شه ⚡"
    ...
    private fun makeInput(hintText: String) = EditText(this).apply {
        hint = hintText
        textSize = 13f
        setSingleLine(true)
    ...
    private fun submit() {
        val price = ExpenseDataUtils.parsePrice(priceInput.text.toString())
        if (price <= 0) {
