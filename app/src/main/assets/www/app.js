(function() {
  ...
  var editModal = document.getElementById('em'),
    ...
    editReviewModal = document.getElementById('er'),
    editReviewSummary = document.getElementById('ers'),
    editReviewCancelButton = document.getElementById('erc'),
    editReviewYesButton = document.getElementById('ery');
  ...
  function saveExpenses() {
    ...
    try {
      if (window.AndroidBridge && typeof AndroidBridge.syncExpenses === 'function') {
        AndroidBridge.syncExpenses(JSON.stringify(expenses))
      }
    } catch (e) {}
  }
  ...
  function renderExpenses() {
    ...
    var ks = Object.keys(g).sort(function(a,b){var pa=a.split('-'),pb=b.split('-');var ya=+pa[0],yb=+pb[0];if(ya!==yb)return ya-yb;var ma=+pa[1],mb=+pb[1];if(ma!==mb)return ma-mb;return+pa[2]-+pb[2]}),
      h = '',
      idx = 0;
  ...
  function downloadFile(content, type, name) {
    if (window.AndroidBridge && typeof AndroidBridge.saveFile === 'function') {
      AndroidBridge.saveFile(String(name || 'expenses.txt'), String(type || 'text/plain'), String(content || ''));
      return
    }
  ...
  editDateInput.addEventListener('click', function() {
    // تقویم جای کیبورد را می‌گیرد؛ کیبورد نرم را پیش از باز شدن می‌بندیم.
  ...
  function describeEditChanges(oldItem, nt, np, nd) {
    var changes = [];
    if (oldItem.t !== nt) changes.push('عنوان: «' + escapeHtml(oldItem.t) + '» ← «' + escapeHtml(nt) + '»');
    if (oldItem.p !== np) changes.push('مبلغ: ' + formatNumber(oldItem.p) + ' ← ' + formatNumber(np) + ' تومان');
    if (oldItem.d !== nd) changes.push('تاریخ: ' + formatDate(oldItem.d) + ' ← ' + formatDate(nd));
    return changes;
  }
  editSaveButton.addEventListener('click', function() {
    ...
    var nd = parseDisplayedDate(editDateInput.value.trim()) || expenses[editingDataIndex].d;
    var changes = describeEditChanges(expenses[editingDataIndex], nt, np, nd);
    if (!changes.length) { closeEditModal(); return; }
    pendingEdit = { t: nt, p: np, d: nd };
    editReviewSummary.innerHTML = '<strong>این تغییرات اعمال می‌شوند:</strong><br>' + changes.join('<br>');
    editReviewModal.classList.add('show');
  })
  editReviewYesButton.addEventListener('click', function() {
    if (editingDataIndex < 0 || !pendingEdit) return;
    expenses[editingDataIndex].t = pendingEdit.t;
    expenses[editingDataIndex].p = pendingEdit.p;
    expenses[editingDataIndex].d = pendingEdit.d;
    pendingEdit = null;
    editReviewModal.classList.remove('show');
    saveExpenses(); resetSelection(); renderExpenses(); closeEditModal()
  });
  editReviewCancelButton.addEventListener('click', function() { pendingEdit = null; editReviewModal.classList.remove('show') });
  editReviewModal.addEventListener('click', function(e) { if (e.target === editReviewModal) { pendingEdit = null; editReviewModal.classList.remove('show') } });
  editPriceInput.addEventListener('keydown', function(e) {
    if (e.key === 'Enter') { e.preventDefault(); editDateInput.click(); }
  });
  ...
  selMoveButton.addEventListener('click', function() {
    ...
    renderCalendar(function(k) {
      ...
      var targetDate = formatDate(k);
      showDeleteConfirmation(function() {
        moveSelectedTo(k)
      }, '<div style="font-size:14px;color:#2a2a2a;margin-bottom:4px">' +
        toPersianDigits(selectedCount) + ' مورد انتخاب شده</div>' +
        '<div style="font-size:12px;color:#999">موارد انتخاب‌شده به تاریخ ' + targetDate + ' منتقل شوند؟</div>',
        null)
    });
    calendarModal.classList.add('show')
  });
  ...
  document.addEventListener('keydown', function(e) {
    ...
    if (confirmModal.classList.contains('show')) {
      ...
      confirmCancelCallback = null;
      pendingConfirmCallback = null
    } else if (editReviewModal.classList.contains('show')) {
      pendingEdit = null;
      editReviewModal.classList.remove('show')
  ...
  function refreshFromAndroidBridge() {
    try {
      if (!window.AndroidBridge || typeof AndroidBridge.getExpensesJson !== 'function') return;
      var data = AndroidBridge.getExpensesJson();
      if (!data) return;
      var arr = JSON.parse(data);
      if (!Array.isArray(arr)) return;
      expenses = normalizeImportedItems(arr);
      nextExpenseId = 0;
      ensureItemIds();
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(expenses));
  ...
  try { window.__androidRefreshFromNative = refreshFromAndroidBridge } catch (e) {}

  function applyAndroidWidgetOpenMode() {
    var mode = '';
    try {
      mode = localStorage.getItem('android_widget_open_mode') || '';
      localStorage.removeItem('android_widget_open_mode')
    } catch (e) {}
    if (!mode) return;
    ...
    if (mode === 'month') {
      filterFrom = firstOfMonth();
      filterTo = today;
      return
    }
    if (mode === 'week') {
      var wd = (new Date().getDay() + 1) % 7; // شنبه=۰، جمعه=۶
      var g = new Date();
      g.setDate(g.getDate() - wd);
      var s = gregorianToJalali(g.getFullYear(), g.getMonth() + 1, g.getDate());
      filterFrom = s.y + '-' + s.m + '-' + s.d;
      filterTo = today
    }
  }

  loadExpenses();
  applyAndroidWidgetOpenMode();
  renderExpenses();
